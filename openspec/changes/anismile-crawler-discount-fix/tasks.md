...
- [ ] 修改 `parseProductApi` 折扣解析順序，實作「Discount rate must be correctly parsed from source API」
...
- [ ] 確認 `crawlAnismileProductsWithStats` 折扣比率警告已存在（實作「Crawl result validates discount coverage」）
...
- [ ] 確認 `packages/sync/src/crawler.ts` sync log 折扣統計已存在（實作「Sync log records discount statistics」）
...
- [ ] [P] 在 `packages/api/modules/anismile/lib/crawler.test.ts` 新增折扣解析單元測試，驗證「price_percent 為 primary」規格
...
- [ ] [P] 建立 `~/.claude/skills/external-api-crawler-audit/SKILL.md`，實作三項規格：「External API crawler must include field inventory before implementation」、「External API crawler must validate critical field coverage after crawl」、「Crawler skill must be invoked for all external API crawler tasks」
...
- [ ] [P] 建立 `~/.claude/skills/nextjs-image-audit/SKILL.md`
...
- [ ] 執行 `pnpm --filter @repo/api test` 確認全綠
- [ ] 執行 `pnpm type-check` 確認無型別錯誤
- [ ] 執行 `pnpm build` 確認成功
- [ ] `git add`、`git commit -m "fix(anismile): use price_percent as primary discount source"`
