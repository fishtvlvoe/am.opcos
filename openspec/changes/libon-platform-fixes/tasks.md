...
- [ ] [P] 1.1 SeriesCard 使用 Next.js Image 載入系列圖片（實現 "Series cards use optimized image loading" requirement）
...
- [ ] 4.1 getSourceSeriesImageMap 並發數從 30 降至 7（實現 "Limit concurrent source API requests" requirement）
...
- [ ] 5.1 重構 getSeriesList 優先使用原始網站數據（實現 "Date tab switching reflects correct series list" requirement）
...
- [ ] 5.2 確保原始 API 失敗時正確 fallback 到本地 DB（實現 Fallback to local DB when source API is unavailable requirement）
...
- [ ] 5.3 移除不必要的 shouldUseSyncedDateFallback 複雜判斷（實現 Date tab switching reflects correct series list requirement）
...
- [ ] 6.1 crawlAnismileProductsBySeriesName limit 從 60 提高到 200（實現 "Series detail page shows complete product count" requirement）
...
- [ ] 6.2 listProducts 即時爬蟲改為背景觸發（實現 "Background crawling for missing series products" requirement）
...
- [ ] 6.3 改進系列名稱匹配邏輯（實現 "Improved series name matching" requirement）
...
- [ ] 7.1 Lighthouse 性能基準測試（驗證 design 中「行為」第一條與「驗收標準」第一條：LCP 顯著改善）
...
- [ ] 7.2 日期切換手動驗收測試（驗證 design 中「驗收標準」第二條）
...
- [ ] 7.3 系列商品數量一致性驗收測試（驗證 design 中「驗收標準」第三條）
...
- [ ] 8.1 移除搜尋頁無查詢分支的 onlyInStock 限制（實現 "Search page displays all synced products by default" requirement）
...
- [ ] 8.2 確認關鍵字搜尋範圍同步擴大（實現 "Keyword search behavior is unchanged" requirement）
...
- [ ] 9.1 新增 getDeadlineList API procedure 呼叫 /deadline_list/index（實現 Homepage displays an "Upcoming Deadlines" section sourced from anismile.jp requirement）
...
- [ ] 9.2 重構現有 DeadlineSection.tsx 改用系列卡片（實現 "Deadline date display" requirement）
...
- [ ] 9.3 在 app/(public)/page.tsx 預取截單列表數據（實現 section integration requirement）
...
- [ ] 10.1 新增 getInstockList API procedure 呼叫 /instock/index（實現 Homepage displays a "In-Stock Products" section sourced from anismile.jp requirement）
...
- [ ] 10.2 新增首頁「現貨銷售」前端 section 組件（實現 "Price display based on login state" requirement）
...
- [ ] 10.3 將 InstockSection 加入 HomePage.tsx（實現 "Source API unavailable" failure requirement）
